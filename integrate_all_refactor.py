...
def run_experiment(project: str, fuzz_target: str, date: str, timeout: int) -> typing.Tuple[bool, typing.List[subprocess.Popen]]:
    successful = True
...
def main() -> typing.Tuple[int, dict]:
    # signal for timeout
...
for project, (fuzz_target_list, date) in OSS_LIBRARIES.items():
    for fuzz_target in fuzz_target_list:
        ...
        try:
            ...
            cmd2 = f"source .venv/bin/activate; " \
                   f"PYTHONPATH=. python3 benchmarks/oss_fuzz_benchmark_integration.py " \
                        f"-p {project} " \
                        f"-f {fuzz_target} " \
                        f"-d {date} " \
            ...
            p = subprocess.run(cmd2, cwd=FUZZBENCH_DIRECTORY, shell=True)
            if p:
