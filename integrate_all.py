...
RUN_NAME = "integrate_all_aflplusplus_4"
...
def main() -> int:
    ...
    runner = ExpRunner(test_run_timeout=TEST_RUN_TIMEOUT, fuzzbench_path=FUZZBENCH_DIRECTORY, save_path=SAVE_DIRECTORY,
                       logger=logger, base_commit='43873069112a742fe9cdf8c0955098c1539aea7a')
    ...
    try:
        ...
        for project in OSS_LIBRARIES.keys():
            ...
            for fuzz_target in fuzz_targets:
                ...
                if system_pruned:
                    runner.run_experiment(project, fuzz_target, commit_hash, commit_date,
                                          timeout=2 * TEST_RUN_TIMEOUT, fuzzer='aflplusplus')
                    system_pruned = False
                else:
                    runner.run_experiment(project, fuzz_target, commit_hash, commit_date, fuzzer='aflplusplus')
