...
LIB_PATH = "/home/forian/uni/integrate_all_aflplusplus_4"
...
def get_fuzzing_exp(simple: bool = False):
    ...
    for lib in libs:
        has_worked = False
        lib_name, fuzz_target, commit_hash, date = lib.split('__')

        debug_print(f'{lib_name}: ({fuzz_target}, {commit_hash}, {date})\n')
    ...
    with open('../results/oss-fuzz_fail_fuzzing', 'a') as f:
    ...
    with open('../results/oss-fuzz_others_fuzzing', 'a') as f:
    ...
    should_write_others = False  # TODO
    with open('../results/oss-fuzz_others_fuzzing', 'a') as f:
